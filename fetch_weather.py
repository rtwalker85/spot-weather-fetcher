...
NOTE = (
    "Different models publish different variables -- a location's entry for "
    "a given model only includes what that model actually provides (e.g. "
    "only HRRR/RAP/NAM/GFS publish a direct cloud-ceiling height and "
    "freezing level; HRDPS/RDPS/GDPS do not). model_elevation_m is what "
    "that model's own terrain grid thinks the elevation is at the matched "
    "point -- compare against the location's real elevation_m in "
    "locations.json to gauge how much a coarse grid may be smoothing over "
    "sharp terrain. Precipitation TYPE is only included for the US bundled "
    "models (rain/snow/freezing_rain/ice_pellets flags are unambiguous); "
    "the Canadian per-file models' PTYPE-style numeric codes are deferred "
    "until verified. A null cloud_ceiling_m means the model found no "
    "defined cloud base (effectively clear / unlimited ceiling), not that "
    "the reading is missing. cape_jkg is null when the model reports a "
    "negative sentinel (no instability data) rather than a real value, "
    "since CAPE can't physically be negative. Any location with "
    "active=false in locations.json is a placeholder and is not included "
    "below."
)
